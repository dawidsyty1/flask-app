...
class BillingCycleError(NotFound):
    pass
...
class BillingCycle(db.Model):
    ...
    @classmethod
    def current_billing_cycle(cls):
        """Retrieve current billing cycle from database"""
        current_cycle = BillingCycle.get_current_cycle()
        if not current_cycle:
            raise BillingCycleError("Error, not found current billing cycle")
