...
class SubscriptionError(NotFound):
    pass
...
class Subscription(db.Model):
    ...
    @classmethod
    def get_and_validate_subscription(cls, sid):
        """Retrieve from database and validate fields"""
        subscription = Subscription.query.get(sid)

        if not subscription:
            raise SubscriptionError(f"Error, not found subscription with id : {sid}")

        if subscription.status is SubscriptionStatus.new:
            raise SubscriptionError(f"Error, subscription with id : {sid} is new")

        if not subscription.data_usages:
            raise SubscriptionError(f"Error, not found data usages for the subscription with id : {sid}")

        if not subscription.plan:
            raise SubscriptionError(f"Error, not found plan for the subscription with id : {sid}")
